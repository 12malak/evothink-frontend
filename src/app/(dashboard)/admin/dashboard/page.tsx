...
const stats = [
  { label:"Total Students",       labelAr:"إجمالي الطلاب",      value:"3,482",  change:"+12%",  positive:true,  color:"#107789", bg:"#EBF5F7",
  ...
  { label:"Active Teachers",      labelAr:"المعلمون النشطون",    value:"148",    change:"+4%",   positive:true,  color:"#7c3aed", bg:"#ede9fe",
  ...
  { label:"Total Revenue",        labelAr:"إجمالي الإيرادات",   value:"$84,230", change:"+8.3%", positive:true,  color:"#059669", bg:"#d1fae5",
  ...
  { label:"Pending Enrollments",  labelAr:"التسجيلات المعلقة",  value:"57",     change:"-3%",   positive:false, color:"#d97706", bg:"#fef3c7",
...
// ── أسماء الأشهر كاملة ─────────────────────────────────────────
const monthlyData = [
  { month:"Sep", monthAr:"سبتمبر",  value:60 },
  { month:"Oct", monthAr:"أكتوبر",  value:75 },
  { month:"Nov", monthAr:"نوفمبر",  value:55 },
  { month:"Dec", monthAr:"ديسمبر",  value:80 },
  { month:"Jan", monthAr:"يناير",   value:70 },
  { month:"Feb", monthAr:"فبراير",  value:90 },
...
// ── Bar Chart — labels مائلة عند العربية ──────────────────────
function BarChart({ isRTL }: { isRTL: boolean }) {
  ...
  const topPad = 28;    // space above tallest bar
  // عند العربية نزيد المساحة السفلية لأسماء أطول (مائلة)
  const botPad = 32;
  const totalW = monthlyData.length * (bW + gap) - gap;
  const vb     = `0 -${topPad} ${totalW + 4} ${cH + topPad + botPad}`;
    ...
    <div className="w-full overflow-x-auto">
      ...
      >
        {monthlyData.map((d, i) => {
          ...
          // نقطة ارتساء النص — وسط العمود
          const lx = x + bW / 2;
          const ly = cH + (isRTL ? 14 : 18);
          ...
          return (
            <g key={i}>
              ...
              <rect
                ...
                style={{ transition:`height .7s ${i*.08}s ease, y .7s ${i*.08}s ease` }}
              />

              {/* Pill — top bar */}
              {isTop && (
                <g>
                  <rect x={x+bW/2-pillW/2} y={labelY-pillH+3} width={pillW} height={pillH} rx={pillH/2} fill="#107789"/>
                  <text x={x+bW/2} y={labelY} textAnchor="middle" fontSize="10" fontWeight="800" fill="white" fontFamily="system-ui,sans-serif">
                    {labelText}
                  </text>
                </g>
              )}
              ...
              {/* Value — other bars */}
              {!isTop && (
                <text x={x+bW/2} y={y-5} textAnchor="middle" fontSize="9" fontWeight="600" fill="#94a3b8" fontFamily="system-ui,sans-serif">
                  {d.value}
                </text>
              )}

              {/* Month label — horizontal always, font smaller for AR */}
              <text
                x={lx}
                y={ly}
                textAnchor="middle"
                fontSize={isRTL ? 7.5 : 11}
                fill="#94a3b8"
...
export default function HomePage() {
  ...
  const statusColor = (s:string) =>
    s==="Full" ? {color:"#d97706",bg:"#fef3c7"} : {color:"#059669",bg:"#d1fae5"};
      ...
      <div className="flex flex-col flex-1 min-w-0">
        <main className="flex-1 p-4 sm:p-6 space-y-5 sm:space-y-6 overflow-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
            {stats.map(s=>(
              <div key={s.label} className="bg-white rounded-2xl p-5 flex gap-4 items-start shadow-sm border border-[#F1F5F9] hover:shadow-md hover:border-[#b2dce4] transition-all duration-300">
                <div className="rounded-xl p-3 flex-shrink-0" style={{backgroundColor:s.bg,color:s.color}}>{s.icon}</div>
                <div className="min-w-0">
                  ...
                  <p className="text-xs mt-1 font-medium" style={{color:s.positive?"#059669":"#ef4444"}}>
                    {s.change} <span className="text-[#94a3b8] font-normal">{t("vs last month","مقارنة بالشهر الماضي")}</span>
                  </p>
                </div>
              </div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#F1F5F9] hover:shadow-md transition-all duration-300">
                ...
                return(
                  <div key={i} className="p-4 space-y-2">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-bold text-[#1e293b] truncate">{t(c.name,c.nameAr)}</p>
                      <span className="flex-shrink-0 rounded-full px-2.5 py-0.5 text-[11px] font-bold" style={{backgroundColor:sc.bg,color:sc.color}}>{t(c.status,c.statusAr)}</span>
                    </div>
                    <p className="text-xs text-[#94a3b8]">{t(c.teacher,c.teacherAr)}</p>
                    <div className="flex items-center gap-2">
                      <ProgressBar value={c.completion} color="#107789"/>
                      <span className="text-xs font-bold text-[#107789] whitespace-nowrap">{c.completion}%</span>
                    </div>
                    <p className="text-xs text-[#94a3b8]">{c.enrolled}<span className="text-[#cbd5e1]">/{c.capacity}</span> {t("enrolled","مسجل")}</p>
                  </div>
                );
              })}
            </div>
                  ...
                  {topCourses.map((c,i)=>{
                    ...
                    return(
                      <tr key={i} className="hover:bg-[#F8FAFC] transition-colors border-b border-[#F1F5F9] last:border-0">
                        <td className="px-4 py-3.5 font-semibold text-[#1e293b]">{t(c.name,c.nameAr)}</td>
                        <td className="px-4 py-3.5 text-[#64748b]">{t(c.teacher,c.teacherAr)}</td>
                        <td className="px-4 py-3.5 text-[#475569]">{c.enrolled}<span className="text-[#94a3b8] text-[11px]">/{c.capacity}</span></td>
                        <td className="px-4 py-3.5" style={{minWidth:130}}>
                          <div className="flex items-center gap-2">
                            <ProgressBar value={c.completion} color="#107789"/>
                            <span className="text-[11px] font-bold text-[#107789] whitespace-nowrap">{c.completion}%</span>
                          </div>
                        </td>
                        <td className="px-4 py-3.5">
                          <span className="rounded-full px-2.5 py-0.5 text-[11px] font-bold" style={{backgroundColor:sc.bg,color:sc.color}}>{t(c.status,c.statusAr)}</span>
                        </td>
                      </tr>
                    );
                  })}
            ...
            <div className="sm:hidden divide-y divide-[#F8FAFC]">
              {recentActivity.map((item,i)=>(
                <div key={i} className="flex items-start gap-3 p-4">
                  <div className="w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0" style={{backgroundColor:item.bg,color:item.color}}>{item.avatar}</div>
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="inline-flex mt-1.5 rounded-full px-2 py-0.5 text-[10px] font-bold" style={{backgroundColor:item.bg,color:item.color}}>{t(item.role,item.roleAr)}</span>
                  </div>
                </div>
              ))}
            </div>
                    ...
                    <tr key={i} className="hover:bg-[#F8FAFC] transition-colors border-b border-[#F1F5F9] last:border-0">
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-[13px] flex-shrink-0" style={{backgroundColor:item.bg,color:item.color}}>{item.avatar}</div>
                          <span className="font-semibold text-[#1e293b]">{item.name}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                        <span className="rounded-full px-2.5 py-0.5 text-[11px] font-bold" style={{backgroundColor:item.bg,color:item.color}}>{t(item.role,item.roleAr)}</span>
                      </td>
