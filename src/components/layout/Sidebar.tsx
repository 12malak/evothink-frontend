...
export default function Sidebar(){
  ...
  // ── Accordion — افتح القسم تلقائياً عند أول تحميل ──────────
  ...
  useEffect(()=>{
    ...
    visibleItems.forEach(item=>{
      // افتح إذا كان الـ pathname يطابق أحد الـ children
      const childMatch=item.children?.some(
        c=>pathname===c.href||pathname.startsWith(c.href+"/")
      );
      if(childMatch) auto[item.key]=true;
    });

    // إذا لم يوجد تطابق بعد (مثلاً /admin فقط) — افتح أول قسم تلقائياً
    if(!initialOpened && Object.keys(auto).length===0 && visibleItems.length>0){
      const first=visibleItems.find(i=>i.children);
      if(first) auto[first.key]=true;
    }
  ...
  },[pathname,visibleItems,initialOpened]);
  ...
  return(
    <>
      ...
      <div onClick={closeMobile}
        className={["fixed inset-0 z-40 bg-black/30 backdrop-blur-[1px] transition-all duration-300 md:hidden",
          mobileOpen?"pointer-events-auto opacity-100":"pointer-events-none opacity-0"].join(" ")}/>

      {/* Mobile drawer */}
      <aside className={["fixed top-0 z-50 h-full w-60 bg-white shadow-2xl transition-transform duration-300 ease-out md:hidden",
        mobileEdge, mobileOpen?"translate-x-0":mobileHidden].join(" ")}
        dir={isRTL?"rtl":"ltr"}>
